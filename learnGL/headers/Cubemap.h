...
public:
	...
	Cubemap(std::string path, unsigned int textureResolution, bool GenerateConvolutedMaps)
	{
		...
		equirectangularToCubemap(colorCubemapID,textureResolution,false);
		

		if (GenerateConvolutedMaps) 
		{
			// irradiance map
			equirectangularToCubemap(irradianceMapID, 32, true);
			// prefilter Map
			equirectangularToPrefilteredCubemap(prefilterMapID, 128);
	...
	void equirectangularToCubemap(unsigned int& cubemapID,unsigned int textureRes, bool makeConvolutedIrradianceMap)
	{
		...
		// fill in filepath
		Shader equirectangularToCubemapShader("shaders/equirectangularToCubemap.vert","shaders/equirectangularToCubemap.frag");
		...
		for(int i = 0;i < 6;i++)
		{
			...
			dummyObj.Render(equirectangularToCubemapShader, glm::mat4(0.0f), glm::mat4(0.0f));
	...
	void equirectangularToPrefilteredCubemap(unsigned int& cubemapID, unsigned int textureRes)
	{
		// to create a cubemap from an equirectangular panorama Hdri image I Create a framebuffer that renders to a texture.
		// the using a clever shader by the khronos group we loop through all 6 sides of the cube/cubemap textures and render each directly to the cubemap object;
		// For this we need said shaders and a dummy object to make a drawcall but as i understad it it doesn't matter what the object is since it's all handled by the shader
		...
		{
			unsigned int mipRes = textureResolution * std::pow(0.5, mip);
	...
	void generateBrdfLUTTexture()
	{
		...
		// fill in filepath
		Shader brdfLUTShader("shaders/equirectangularToCubemap.vert", "shaders/cookTorranceBRDFLut.frag");
		Object dummyObj("fbx/skyboxMesh.fbx");
		...
		dummyObj.Render(brdfLUTShader, glm::mat4(0.0f), glm::mat4(0.0f));
